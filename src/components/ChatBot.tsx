...
const ChatBot: React.FC = () => {
  ...
  useEffect(() => {
    if (isOpen) {
        ...
        const timer = setTimeout(() => setIsRendered(false), 300); // Czas trwania animacji
  ...
  return (
    <>
      ...
      >
        <i className={`fa-solid transition-transform duration-300 ${isOpen ? 'fa-times' : 'fa-comments'}`}></i>
      </button>
      
      {isRendered && (
        <div className={`fixed bottom-44 right-6 w-full max-w-sm h-[60vh] bg-[#16213e] rounded-2xl shadow-2xl flex flex-col z-50 border border-cyan-400/30 transition-all duration-300 ease-out ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
